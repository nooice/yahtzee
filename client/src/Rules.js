...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    //if only 2 sets there must be a 3 of a kind and a 2 of a kind
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    // small straight can equall a large straight or...
    // small straight can be 5 different dice & only one can be a 2 or a 5 or...
    // small straight can be 4 different dice & can not be a 1 and a 6, 1 and a 2, or a 5 and 6
    let large5 = d.size === 5 && (!d.has(1) || !d.has(6));
    let small5 = d.size === 5 && (!d.has(2) || !d.has(5));
    let small4 = d.size === 4 && ( (!d.has(1) && !d.has(6)) || (!d.has(1) && !d.has(2)) || (!d.has(5) && !d.has(6)) )

    return large5 || small5 || small4 ? this.score : 0;
  };
}
